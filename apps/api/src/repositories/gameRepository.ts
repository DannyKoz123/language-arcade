...
export class GameRepository {
  ...
  async lockPlayer(playerId: string, executor: Queryable = this.pool): Promise<void> {
    await executor.query(`select id from players where id = $1 for update`, [playerId]);
  }

  async getActiveSeason(executor: Queryable = this.pool): Promise<SeasonRecord> {
  ...
  async getCatalogSnapshot(): Promise<CatalogSnapshot> {
    ...
    return {
      languages: languagesResult.rows.map(mapLanguage),
      clips: clipsResult.rows.map(mapClip),
  ...
  async getLatestActiveRunForPlayer(
    playerId: string,
    executor: Queryable = this.pool
  ...
  async getRun(
    runId: string,
    options: { executor?: Queryable; lock?: boolean } = {}
  ): Promise<RunRecord | null> {
  ...
  async listRunRounds(
    ...
    const result = await executor.query<RunRoundRow>(
    ...
    return result.rows.map(mapRunRound);
  }
  ...
  async getRunRound(
    ...
    return result.rows[0] ? mapRunRound(result.rows[0]) : null;
  }

  async insertRunRound(
    input: {
      runId: string;
      roundNumber: number;
      languageIsoCode: string;
    ...
    return mapRunRound(result.rows[0]);
  }
    ...
    await executor.query(
      `update runs
       set score = $2,
           ...
           status = $7,
           finished_at = case
             when $7 in ('completed', 'abandoned') then coalesce(finished_at, now())
             else null
           end
       where id = $1`,
      [
  ...
  async applyHint(
    roundId: string,
    hintType: "family_region" | "longer_clip",
    executor: Queryable = this.pool
  ): Promise<RunRoundRecord | null> {
  ...
  async publishContentVersion(versionName: string): Promise<void> {
    await this.withTransaction(async (client) => {
      ...
      await client.query(
        `update languages
         set enabled = (content_version_id = $1)`,
        [version.id]
      );
  ...
  async disableClip(clipId: string): Promise<boolean> {
...
function mapRunRound(row: RunRoundRow): RunRoundRecord {
  return {
    id: row.id,
    runId: row.run_id,
